      ...
      <div class="row">
        <div class="col-sm-5 text-center">
          <small class="text-muted inline m-t-sm m-b-sm">
            {{ "Hiển thị ". $all_category_product->firstItem() ."-". $all_category_product->lastItem() ." trong ". $all_category_product->total() ." dòng dữ liệu" }}
          </small>
        </div>
        <nav aria-label="Page navigation">
          <div class="col-sm-7 text-right text-center-xs">
            <ul class="pagination pagination-sm m-t-none m-b-none">
              ...
              @else
                <li><a href="{{ $all_category_product->previousPageUrl() }}">Trước</a></li>
              @endif

              {{-- Pagination Elements --}}
              @for ($key=0; $key+1<=$all_category_product->lastPage(); $key++)
                @if ($all_category_product->currentPage() === $key + 1)
                  <li><a href="{{ $all_category_product->url($key + 1) }}" style="color:#fff; background-color: #ffbd0b">{{ $key + 1 }}</a></li>
                @else
              ...
              @if ($all_category_product->hasMorePages())
                <li><a href="{{ $all_category_product->nextPageUrl() }}">Sau</a></li>
              @else
